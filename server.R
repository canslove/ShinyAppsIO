...
server <- function(input, output, session) {
    ...
    output$table1 <- DT::renderDataTable({
      datatable(raw.df[input$slider0[1]:input$slider0[2],], rownames=FALSE) %>% 
    ...
    output$table2 <- DT::renderDataTable({
      datatable(raw.df[raw.df[,"DATE"]==input$Id_date[1],], rownames=FALSE) %>% 
        #datatable(raw.df4dt()[input$slider0[1]:input$slider0[2],], rownames=FALSE) %>% 
        #caption = 'Table 1: This is a simple caption for the table.' %>%
        formatStyle(input$selected,  
                    background="skyblue", fontWeight='bold')
    ...
    output$desc <- renderText({
      ...
      trend_text <- 'Test :'
      paste(trend_text, "The data is Top100 ranked songs from Jan 2017 to Feb 2018 @ Spotify.")
    }) #output$desc <- renderText
